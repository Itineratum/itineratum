...
export const planIsValid = (validatePlanRes: any) => {
  return (
...
export const runPipelineWithGenerationSteps = async (
  ...
  try {
    ...
    if (planIsValid(validatePlanRes)) {
      // validate again
      const validatePlanRes2 = await validatePlan(itineraryJson);

      if (planIsValid(validatePlanRes2)) {
        setGenerationStep(GenerateItineraryStep.searchingHotels);
      } else {
        throw new Error(validatePlanRes2.invalid_reason);
      }
    } else {
